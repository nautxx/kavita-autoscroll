...
script_version="$(sed -n "s/^\/\/ @version[[:space:]]*//p" "$repo_dir/scrollito.user.js")"
requested_version="${1:-$manifest_version}"
release_dir="$repo_dir/build/release/v$requested_version"
archive_name="scrollito-v$requested_version.zip"
...
rm -rf "$release_dir"
mkdir -p "$release_dir"
cp "$repo_dir/scrollito.user.js" "$release_dir/"

(
  cd "$repo_dir"
  zip -q "$release_dir/$archive_name" manifest.json scrollito.user.js README.md LICENSE
)

(
  cd "$release_dir"
  shasum -a 256 scrollito.user.js "$archive_name" > SHA256SUMS.txt
)
