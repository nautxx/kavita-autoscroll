...
docker build \
  ...
  --build-arg AUTOSCROLL_VERSION=1.2.0 \
  --tag ghcr.io/nautxx/scrollito-injector:1.2.0 \
  .
```
...
versioned `build/release/v1.2.0/` directory. The injector image uses the
userscript at the repository root as its source, so the browser and server
packages stay on the same version.

Pushing a matching tag such as `v1.2.0` runs the release workflow. It publishes
